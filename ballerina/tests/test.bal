...
configurable string serviceUrl = isLiveServer ? "https://api.hubapi.com/crm/v3/objects/taxes" : "http://localhost:9090";
...
final Client taxes = check new Client(config, serviceUrl);
...
function testPostBatchUpdate() returns error? {
    ...
    BatchResponseSimplePublicObject|BatchResponseSimplePublicObjectWithErrors response = check taxes->/batch/update.post(payload);
